...
import (
	...
	"github.com/zercle/go-angular-spa-template/internal/testutil"
	"github.com/zercle/go-angular-spa-template/internal/testutil/fixtures"
)

func TestNewRequest_JSON(t *testing.T) {
	req := testutil.NewRequest(http.MethodPost, "/tasks", bytes.NewReader([]byte(`{}`)))
...
func TestFixtures_NewTask(t *testing.T) {
	item := fixtures.NewTask("fixture")
	assert.Equal(t, "fixture", item.Title)
